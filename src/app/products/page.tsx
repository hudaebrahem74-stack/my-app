        ...
        <Link key={prod._id} href={`/products/${prod._id}`}>
          <div className="group border p-3 rounded-lg shadow hover:shadow-lg transition flex flex-col cursor-pointer relative">
            ...
            <div className="text-center mt-1">
              {prod.priceAfterDiscount ? (
                <>
                  <p className="text-gray-400 line-through">{prod.price} EGP</p>
                  <p className="text-green-600 font-bold">{prod.priceAfterDiscount} EGP</p>
                </>
              ) : (
                <p className="text-green-600 font-bold">{prod.price} EGP</p>
              )}
            </div>
